  ...
  def evaluate_data_message(msg)
    if @node_type == "L"
      if msg["typ"] == "ack"   # Bestätigung der Aufteilung 
        ...
        send_message_to_neighbours(generate_request_msg)  # Sende Request Nachricht an andere Knoten
      end     
    elsif @node_type == "F"
        if accept_request(msg["value"].to_i) # Anfrage passt zur Strategy
          ...
          send_ack(msg["source"].to_i)  # Sende Bestätigung an Leader
        else 
...
begin
  Thread.new{  # Es wird ein eigener Thread gestartet der die Queue abarbeitet
    ...
    while 1
      node.evaluate_queue_msg unless node.is_empty # Arbeite Queue ab solange sie nicht leer ist 
    end   
  }
  while 1
